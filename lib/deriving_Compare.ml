...
module Compare_alpha(S : sig type a end) = struct type a = S.a let compare _ _ = assert false end
...
module Compare_int32 = Compare_immutable(struct type a = int32 end)
module Compare_int64 = Compare_immutable(struct type a = int64 end)
module Compare_nativeint = Compare_immutable(struct type a = nativeint end)
